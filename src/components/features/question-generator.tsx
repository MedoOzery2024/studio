...
export function QuestionGenerator() {
    ...
    const startQuiz = (session?: SavedSession) => {
        ...
        const initialAnswers = targetSession.userAnswers || [];
        const isCompleted = initialAnswers.length === targetSession.questions.length && initialAnswers.length > 0;

        setGeneratedSession(targetSession);
        setQuizState({ status: isCompleted ? 'completed' : 'in-progress', answers: initialAnswers });
        setCurrentQuestionIndex(0);
        setSelectedAnswer('');
        setIsAnswerSubmitted(false);
    };
